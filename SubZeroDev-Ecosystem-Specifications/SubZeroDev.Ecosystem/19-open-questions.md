...
(assume single-repo) and whether the low-level GitHub client is shared with the GitHub plugin.
...
11. Should a ruleset be a named, reusable policy shared across repositories rather than restated in
...
| Does the WorkItems library expose tracker providers?          | **Yes.** Convergence _is_ the write path; reconciliation that cannot execute its actions hands each consumer the half where all four known bugs lived. The approval gate stays with the calling plugin                                                   |
| Does repository provisioning belong to the GitHub plugin?     | **No** — least privilege. The GitHub plugin reads and runs continuously; provisioning needs administration write and runs once. Folded together, the continuously-running plugin declares a capability it never uses, and the declaration _is_ the grant |
