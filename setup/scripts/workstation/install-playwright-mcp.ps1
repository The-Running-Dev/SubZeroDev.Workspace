...
$serverArguments = @('/c', 'npx', '-y', '@playwright/mcp@latest', '--isolated')
...
if ($Client -in @('Codex', 'Both')) {
    ...
    elseif ($PSCmdlet.ShouldProcess('Codex', 'Register Playwright MCP server')) {
        Invoke-NativeCommand -FilePath 'codex' -ArgumentList (@('mcp', 'add', 'playwright', '--', 'cmd') + $serverArguments)
    }
}
...
if ($Client -in @('ClaudeCode', 'Both')) {
    ...
    elseif ($PSCmdlet.ShouldProcess('Claude Code', 'Register Playwright MCP server')) {
        Invoke-NativeCommand -FilePath 'claude' -ArgumentList (@('mcp', 'add', '--scope', 'user', 'playwright', '--', 'cmd') + $serverArguments)
    }
}
